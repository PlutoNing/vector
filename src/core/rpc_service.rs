...
pub struct QueryServiceImpl {
    sqlite_service: crate::sinks::util::sqlite_service::SqliteService,
}

impl QueryServiceImpl {
    pub async fn new(db_path: &str, table_name: &str) -> Result<Self, Box<dyn std::error::Error>> {
        let sqlite_service = crate::sinks::util::sqlite_service::SqliteService::new(db_path, table_name).await?;
...
impl QueryService for QueryServiceImpl {
    async fn query_events(
        ...
        let returned_count = event_data.len() as i32;
...
impl QueryServiceImpl {
    pub async fn start_server(db_path: &str, table_name: &str, socket_path: &str) -> Result<(), Box<dyn std::error::Error>> {
        // 删除已存在的socket文件
        ...
        let query_service = QueryServiceImpl::new(db_path, table_name).await?;
